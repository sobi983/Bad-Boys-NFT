...
import whiteListAddresses from "./FinalWL";
import Web3 from "web3";

const CONTRACT_ADDRESS = "0xB84CBAF116eb90fD445Dd5AeAdfab3e807D2CBaC";
...
export function Mint() {
	...
	async function mint() {
		...
		const ct = new web3.eth.Contract(contractInterface, CONTRACT_ADDRESS);
		console.log(account, "account");
		const sentAddress = keccak256(account.address);
		const leafNodes = whiteListAddresses.map((addr) => keccak256(addr));
		const merkleTree = new MerkleTree(leafNodes, keccak256, {
		...
		const rootHash = merkleTree.getHexRoot();
		const merkleProof = merkleTree.getHexProof(sentAddress);
		console.log(rootHash, "roothash", merkleProof, "Proof");

		await ct.methods
			.WhiteListMint(merkleProof, 1)
	...
	return (
		<>
			<div id="mint" className="h-full">
				<div className="container mx-auto min-h-screen">
					<div className="  md:grid grid-cols-12 min-h-screen">
						<div className="col-span-4 grid-col-2 min-h-screen">
							<div className="flex flex-col justify-between items-center min-h-screen">
								<div>
									<img src="/lamp.png" alt="lamp" className="h-56" />
								</div>
								<div className="mb-20">
									<img src="/nft.gif" alt="nft" className="h-96 " />
								</div>
							</div>
						</div>
						<div className="col-span-8 pt-10 relative   min-h-screen overflow-hidden">
							<div className="absolute -z-10 md:right-20 ">
								<img
									src="./logofull.svg"
									alt=""
									className="h-full -z-10 md:w-9/12"
								/>
							</div>
							<div className="absolute -z-10  bottom-0  sm:right-10 sm:top-96 pt-10">
								<img src="./project.png" alt="" className="h-36 -z-10" />
							</div>
							<div className="flex flex-col mx-10 justify-evenly h-full pb-20">
								<div className="sm:ml-20">
									<img
										src="./skulls.svg"
										alt="skullz"
										className="h-36 -z-20 mt-40"
									/>
								</div>
								<div className=" sm:ml-20">
									<p className="font-xs sm:max-w-xs">
										Click on the button-key below to transfer one{" "}
										<strong className="font-bold">Bad Guy</strong> fugitive to
										your wallet.
									</p>
									{minted ? (
										<>
											...
											<p>
												<strong>
													<a
														href="https://opensea.io/collection/badguysbyrpf"
														target="_blank"
														rel="noreferrer"
									...
									{accountReady ? (
										<>
											<>
												<button
													type="button"
													className="text-gray-900 bg-[#d2ad47] px-10 hover:bg-gray-100 border border-gray-200  focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600  mr-2 mb-2"
												>
													<svg
														version="1.2"
														className="h-6 w-10"
													...
													>
														...
														<path
															fillRule="evenodd"
															d="m182.5 54.4h-136.1v23.5h-6.6v-9.2h-7.7v-9.2h-10.2v9.2h-15.3v-14.3h-6.6v-16.8c0 0 5.6 1 6.1 0.5h176.2q0.2-1.6 0.5-3.2c3.9-19.5 18.4-34.1 35.7-34.1 20.3 0 36.8 20 36.8 44.7 0 24.6-16.5 44.6-36.8 44.6-17.7 0-32.5-15.3-36-35.7zm11.3-16.3q-0.8 3.6-0.8 7.4c0 17.1 11.4 31 25.5 31 14.1 0 25.5-13.9 25.5-31 0-17.1-11.4-31-25.5-31-12 0-22 10.1-24.7 23.6z"
														/>
													</svg>{" "}
													SOLD OUT!
												</button>
										...
										<>
											<div className="text-gray-900 bg-[#d2ad47] hover:bg-gray-100 border border-gray-200  focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5  text-center inline-flex items-center dark:focus:ring-gray-600  mr-2">
												<svg
													version="1.2"
													className="h-6 w-10"
													xmlns="http://www.w3.org/2000/svg"
